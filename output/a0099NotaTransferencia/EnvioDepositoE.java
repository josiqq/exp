...
public class EnvioDepositoE {
   ...
   public static void buscarEnvioTablaReporte(
      ...
      try {
         ...
         preparedStatement2 = conexion.prepareStatement(
            "select\r\nenvios.nro_registro,\r\nenvios.fecha,\r\nenvios.cod_deposito_origen,\r\n(select depositos.nombre_deposito from depositos where envios.cod_deposito_origen = depositos.cod_deposito) as nombre_deposito_origen,\r\nenvios.cod_deposito_destino,\r\n(select depositos.nombre_deposito from depositos where envios.cod_deposito_destino = depositos.cod_deposito) as nombre_deposito_destino,\r\nenvios.recibido\r\nfrom\r\nenvios,envios_detalle\r\nwhere\r\n(envios.nro_registro = envios_detalle.nro_registro)\r\nand (envios.transferencia =1)\r\nand ((envios.recibido =?) or (?=2))\r\nand ((envios.cod_deposito_origen =?) or (?=0))\r\nand ((envios.cod_deposito_destino =?) or (?=0))\r\nand (envios.fecha between (?) and (?))"
         );
         preparedStatement2.setInt(1, estado);
         preparedStatement2.setInt(2, estado);
         preparedStatement2.setInt(3, cod_deposito_origen);
         preparedStatement2.setInt(4, cod_deposito_origen);
         preparedStatement2.setInt(5, cod_deposito_destino);
         preparedStatement2.setInt(6, cod_deposito_destino);
         preparedStatement2.setString(7, fecha_inicial);
         preparedStatement2.setString(8, fecha_final);
         ...
         while (rs2.next()) {
            ...
            tabla.setValueAt(rs2.getInt("nro_registro"), fila, tabla.getColumn("Nro. Registro").getModelIndex());
            tabla.setValueAt(rs2.getString("fecha"), fila, tabla.getColumn("Fecha").getModelIndex());
            tabla.setValueAt(
               rs2.getInt("cod_deposito_origen") + " - " + rs2.getString("nombre_deposito_origen"), fila, tabla.getColumn("Deposito Origen").getModelIndex()
            );
            tabla.setValueAt(
               rs2.getInt("cod_deposito_destino") + " - " + rs2.getString("nombre_deposito_destino"), fila, tabla.getColumn("Deposito Destino").getModelIndex()
            );
            tabla.setValueAt(rs2.getBoolean("recibido"), fila, tabla.getColumn("Recibido").getModelIndex());
         }
      } catch (Exception var17) {
         LogErrores.errores(var17, "Error al Cargar Producto..", frame);
      } finally {
   ...
   public static EnvioDepositoE buscarEnvioTransferencia(
      int registro, JinternalPadre frame, TablaDetalleEnvioTransferencia tabla, int par_decimal_cantidad, int par_decimal_importe
   ) {
      ResultSet rs = null;
      ResultSet rs2 = null;
      ...
      ModeloTablaEnvioTransferencia modelo = (ModeloTablaEnvioTransferencia)tabla.getModel();
      modelo.deleteRow(tabla);
      Connection conexion = null;
   ...
   public static int insertarEnviosTransferencia(EnvioDepositoE entidad, JTable tabla, JinternalPadre frame) {
      ...
      if (ValidacionesTabla.validarSinDetalles(tabla, frame)) {
         ...
         try {
            ...
            preparedStatement1 = conexion.prepareStatement(
               "INSERT INTO envios (fecha,hora_envio,cod_deposito_origen,cod_deposito_destino,observacion,transferencia,recibido,fecha_alta,usuario_alta) VALUES (?,curtime(),?,?,?,1,0,now(), SUBSTRING_INDEX(USER(), '@', 1))",
               1
            );
            ...
            for (int fila = 0; fila < tabla.getRowCount(); fila++) {
               if (!String.valueOf(tabla.getValueAt(fila, codigoColumn)).trim().equals("")
                  ...
                  preparedStatement2.setDouble(3, 0.0);
            ...
            try {
               ...
               LogErrores.errores(var23, "Error al insertar Transferencia.", frame);
            }

            LogErrores.errores(var24, "Error al insertar Transferencia..", frame);
         } finally {
            new CerrarRecursos(preparedStatement1, preparedStatement2, generatedKeys, frame, "Error al insertar Transferencia..");
            DatabaseConnection.getInstance().cerrarConexion();
         }
   ...
   public static int actualizarEnvioTransferencia(EnvioDepositoE entidad, JinternalPadre frame, JTable tablaInsertar, List<Integer> codigosAEliminar) {
      ...
      if (ValidacionesTabla.validarSinDetalles(tablaInsertar, frame)) {
         try {
            ...
            preparedStatementUpdate = conexion.prepareStatement(
               "UPDATE envios_detalle set cod_producto=?,cantidad_enviado=?,costo=?,tipo_fiscal=?,iva=?,porcentaje_gravado=? where id =?"
            );
            preparedStatement1 = conexion.prepareStatement(
            ...
            for (int fila = 0; fila < tablaInsertar.getRowCount(); fila++) {
               if (!String.valueOf(tablaInsertar.getValueAt(fila, codigoColumn)).trim().equals("")
                  ...
                  if (String.valueOf(tablaInsertar.getValueAt(fila, primarioColumn)).trim().equals("0")) {
                     ...
                     preparedStatementInsert.setDouble(3, 0.0);
   ...
   public static int eliminarEnvioDeposito(EnvioDepositoE entidad, JinternalPadre frame) {
      int registro = entidad.getNro_registro();
      PreparedStatement preparedStatement2 = null;
      PreparedStatement preparedStatement1 = null;
      Connection conexion = null;
   ...
   public EnvioDepositoE(int nro_registro, String fecha, int cod_deposito_origen, int cod_deposito_destino, String observacion) {
      this.nro_registro = nro_registro;
      this.fecha = fecha;
      this.cod_deposito_origen = cod_deposito_origen;
      this.cod_deposito_destino = cod_deposito_destino;
