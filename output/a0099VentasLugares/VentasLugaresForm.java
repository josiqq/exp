...
public class VentasLugaresForm extends JinternalPadre {
   ...
   public void buscar() {
      if (!this.txt_codigo.getText().trim().equals("0") && Integer.parseInt(this.txt_codigo.getText()) != this.SW) {
         VentasLugaresE b = VentasLugaresE.buscarLugarVta(Integer.parseInt(this.txt_codigo.getText().trim()), this);
         if (b == null) {
   ...
   public void guardar() {
      ...
      if (d.isResultadoEncontrado()) {
         VentasLugaresE entidad = this.CargarEntidades();
         if (this.SW == 0) {
            if (entidad != null) {
               int codigo = VentasLugaresE.insertarLugarVta(entidad, this);
               if (codigo != 0) {
            ...
            int codigo = VentasLugaresE.actualizarLugarVta(entidad, this);
            if (codigo != 0) {
   ...
   public void eliminar() {
      if (Integer.parseInt(this.txt_codigo.getText()) == this.SW && this.SW != 0) {
         ...
         d.setVisible(true);
         if (d.isResultadoEncontrado()) {
            VentasLugaresE ent = this.CargarEntidades();
            if (ent != null) {
               int codigo = VentasLugaresE.eliminarLugarVta(ent, this);
               if (codigo != 0) {
   ...
   public void cargarLugarVta(VentasLugaresE e) {
      this.txt_codigo.setValue(e.getCod_lugar());
      this.SW = Integer.parseInt(String.valueOf(e.getCod_lugar()));
      this.txt_nombre.setValue(e.getNombre_lugar());
      if (e.getEstado() == 1) {
   ...
   public VentasLugaresE CargarEntidades() {
      int estado = 0;
      ...
      try {
         return new VentasLugaresE(Integer.parseInt(this.txt_codigo.getText()), this.txt_nombre.getText(), estado);
   ...
   public static void main(String[] args) {
      EventQueue.invokeLater(new Runnable() {
         @Override
         public void run() {
            VentasLugaresForm frame = new VentasLugaresForm();
   ...
   public VentasLugaresForm() {
      this.setTitle("Registro de Lugares de Ventas");
      this.setBounds(100, 100, 347, 354);
         ...
         gl_panel_datos.createParallelGroup(Alignment.LEADING)
            .addGroup(
               gl_panel_datos.createSequentialGroup()
                  ...
                  .addGroup(
                     gl_panel_datos.createParallelGroup(Alignment.LEADING)
                        .addGroup(gl_panel_datos.createSequentialGroup().addGap(22).addComponent(lblNombre))
                        .addGroup(
                           gl_panel_datos.createSequentialGroup().addPreferredGap(ComponentPlacement.UNRELATED).addComponent(this.txt_nombre, -2, 25, -2)
                        )
                  )
      ...
      new BuscadorTablaCargar(this.txt_buscador, this.lblLineasRecuperadasTexto, this.modeloTabla, "LugaresVentas", this);
      this.inicializarObjetos();
      ...
      modeloSeleccion.addListSelectionListener(new ListSelectionListener() {
         ...
         public void valueChanged(ListSelectionEvent e) {
            if (!e.getValueIsAdjusting()) {
               int filaSeleccionada = VentasLugaresForm.this.tabla.getSelectedRow();
               if (filaSeleccionada != -1) {
                  VentasLugaresForm.this.buscarTabla();
               }
            }
         }
      });
