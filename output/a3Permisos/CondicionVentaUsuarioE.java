...
public class CondicionVentaUsuarioE {
   public static void cargarCondicionVentaUsuario(String usuario, TablaCondicionVentaUsuario tabla, JDialogPrincipal frame) {
      ResultSet rs = null;
      ...
      try {
         ...
         preparedStatement = conexion.prepareStatement(
            "select\r\nsys_usuarios_condicion_venta.cod_condicion_venta,\r\ncondiciones_ventas.nombre_condicion,\r\nsys_usuarios_condicion_venta.id\r\nfrom\r\nsys_usuarios_condicion_venta,condiciones_ventas\r\nwhere\r\ncondiciones_ventas.cod_condicion=sys_usuarios_condicion_venta.cod_condicion_venta\r\nand sys_usuarios_condicion_venta.usuario =? "
         );
         preparedStatement.setString(1, usuario);
         rs = preparedStatement.executeQuery();

         while (rs.next()) {
            modelo.addRow(new Object[]{rs.getString("cod_condicion_venta"), rs.getString("nombre_condicion"), rs.getInt("id"), rs.getInt("id")});
         }

         modelo.addDefaultRow();
      } catch (Exception var11) {
         LogErrores.errores(var11, "Error al seleccionar Condicion de Venta de Usuario..", frame);
      } finally {
         new CerrarRecursos(preparedStatement, rs, frame, "Error al seleccionar Condicion de Venta de Usuario..");
         DatabaseConnection.getInstance().cerrarConexion();
      }
   }

   public static int insertarCondicionVentaUsuarios(String usuario, JTable tablaInsertar, JDialogPrincipal frame, List<Integer> codigosAEliminar) {
      ...
      try {
         DatabaseConnection db = DatabaseConnection.getInstance();
         conexion = db.abrirConexion();
         preparedStatementDelete = conexion.prepareStatement("delete from sys_usuarios_condicion_venta where id = ?");
         preparedStatementInsert = conexion.prepareStatement("INSERT INTO sys_usuarios_condicion_venta (usuario,cod_condicion_venta) VALUES (?,?)");
         preparedStatementUpdate = conexion.prepareStatement("UPDATE sys_usuarios_condicion_venta set  cod_condicion_venta=? where id =?");
         ...
         try {
            ...
            LogErrores.errores(var19, "Error al actualizar Condicion de Venta de Usuario.", frame);
         }

         LogErrores.errores(var20, "Error al actualizar Condicion de Venta de Usuario..", frame);
      } finally {
         new CerrarRecursos(
            preparedStatementInsert, preparedStatementUpdate, preparedStatementDelete, frame, "Error al insertar Condicion de Venta de Usuario.."
         );
         DatabaseConnection.getInstance().cerrarConexion();
      }
