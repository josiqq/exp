...
public class CuentasUsuariosE {
   public static void cargarCuentaUsuario(String usuario, TablasUsuariosPermisos tabla, JDialogPrincipal frame) {
      ResultSet rs = null;
      ...
      try {
         ...
         preparedStatement = conexion.prepareStatement(
            "select\r\nsys_usuarios_cuentas.cod_cuenta,cuentas.nombre_cuenta,sys_usuarios_cuentas.id\r\nfrom sys_usuarios_cuentas,cuentas\r\nwhere\r\nsys_usuarios_cuentas.cod_cuenta = cuentas.cod_cuenta and sys_usuarios_cuentas.usuario =? "
         );
         preparedStatement.setString(1, usuario);
         rs = preparedStatement.executeQuery();

         while (rs.next()) {
            modelo.addRow(new Object[]{rs.getString("cod_cuenta"), rs.getString("nombre_cuenta"), rs.getInt("id"), rs.getInt("id")});
         }

         modelo.addDefaultRow();
      } catch (Exception var11) {
         LogErrores.errores(var11, "Error al seleccionar Cuenta de Usuario..", frame);
      } finally {
         new CerrarRecursos(preparedStatement, rs, frame, "Error al seleccionar Cuenta de Usuario..");
         DatabaseConnection.getInstance().cerrarConexion();
      }
   }

   public static int insertarCuentasUsuarios(String usuario, JTable tablaInsertar, JDialogPrincipal frame, List<Integer> codigosAEliminar) {
      ...
      try {
         DatabaseConnection db = DatabaseConnection.getInstance();
         conexion = db.abrirConexion();
         preparedStatementDelete = conexion.prepareStatement("delete from sys_usuarios_cuentas where id = ?");
         preparedStatementInsert = conexion.prepareStatement("INSERT INTO sys_usuarios_cuentas (usuario,cod_cuenta) VALUES (?,?)");
         preparedStatementUpdate = conexion.prepareStatement("UPDATE sys_usuarios_cuentas set  cod_cuenta=? where id =?");
         ...
         try {
            ...
            LogErrores.errores(var19, "Error al actualizar Cuentas de Usuarios.", frame);
         }

         LogErrores.errores(var20, "Error al actualizar Cuentas de Usuarios..", frame);
      } finally {
         new CerrarRecursos(preparedStatementInsert, preparedStatementUpdate, preparedStatementDelete, frame, "Error al insertar Cuentas de Usuarios..");
         DatabaseConnection.getInstance().cerrarConexion();
      }
