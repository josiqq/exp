...
public class TipoDocumentosUsuariosE {
   public static void cargarTipoDocumentoUsuario(String usuario, TablaTipoDocumentosUsuario tabla, JDialogPrincipal frame) {
      ResultSet rs = null;
      ...
      try {
         ...
         preparedStatement = conexion.prepareStatement(
            "select\r\nsys_usuarios_tipo_documentos.cod_tipo_documento,\r\ntipo_documentos.nombre_tipo_documento,\r\nsys_usuarios_tipo_documentos.id\r\n from sys_usuarios_tipo_documentos,tipo_documentos\r\nwhere\r\nsys_usuarios_tipo_documentos.cod_tipo_documento = tipo_documentos.cod_tipo_documento\r\nand sys_usuarios_tipo_documentos.usuario =? "
         );
         preparedStatement.setString(1, usuario);
         rs = preparedStatement.executeQuery();

         while (rs.next()) {
            modelo.addRow(new Object[]{rs.getString("cod_tipo_documento"), rs.getString("nombre_tipo_documento"), rs.getInt("id"), rs.getInt("id")});
         }

         modelo.addDefaultRow();
      } catch (Exception var11) {
         LogErrores.errores(var11, "Error al seleccionar Tipo Documento de Usuario..", frame);
      } finally {
         new CerrarRecursos(preparedStatement, rs, frame, "Error al seleccionar Tipo Documento de Usuario..");
         DatabaseConnection.getInstance().cerrarConexion();
      }
   }

   public static int insertarTipoDocumentosUsuarios(String usuario, JTable tablaInsertar, JDialogPrincipal frame, List<Integer> codigosAEliminar) {
      ...
      try {
         DatabaseConnection db = DatabaseConnection.getInstance();
         conexion = db.abrirConexion();
         preparedStatementDelete = conexion.prepareStatement("delete from sys_usuarios_tipo_documentos where id = ?");
         preparedStatementInsert = conexion.prepareStatement("INSERT INTO sys_usuarios_tipo_documentos (usuario,cod_tipo_documento) VALUES (?,?)");
         preparedStatementUpdate = conexion.prepareStatement("UPDATE sys_usuarios_tipo_documentos set  cod_tipo_documento=? where id =?");
         ...
         try {
            ...
            LogErrores.errores(var19, "Error al actualizar Tipo Documento de Usuario.", frame);
         }

         LogErrores.errores(var20, "Error al actualizar Tipo Documento de Usuario..", frame);
      } finally {
         new CerrarRecursos(preparedStatementInsert, preparedStatementUpdate, preparedStatementDelete, frame, "Error al insertar Tipo Documento de Usuario..");
         DatabaseConnection.getInstance().cerrarConexion();
      }
